...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/ioncore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update ioncore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'ioncore-lib-dash': '^v0.13.17',
          'ioncore-node': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/ioncore-node.json');
