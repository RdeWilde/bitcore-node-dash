...
var Proposal = ioncore.GovObject.Proposal;
...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    if (ioncore.Address.isValid(addresses[i], this.node.network)) {
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of ioncore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of ioncore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of ioncore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to dashd from ioncore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that dashd is undergoing a reindex. ' +
             ...
             'of ioncore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === ioncore.Networks.testnet) {
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = ioncore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    var txid = ioncore.util.buffer.reverse(hash).toString('hex');
 ...
 * @param {String|Address|Array} addressArg - An address string, ioncore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = ioncore.Script.fromAddress(delta.address);
    return {
...
Bitcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in ioncore is most recent to less recent
...
Bitcoin.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof ioncore.Address) {
...
/**
 * Will retrieve a block as a ioncore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = ioncore.Block.fromString(response.result);
...
/**
 * Will get a transaction as a ioncore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addTxlockToTx(tx,result) {
    ...
    if (tx.feeSatoshis >= Bitcoin.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.001 check for txlock
      var rawTx = new ioncore.Transaction(result.hex);
      var hash = ioncore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
