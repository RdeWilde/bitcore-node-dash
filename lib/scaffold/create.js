...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with ioncore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'ioncore-lib-dash': '^' + ioncore.version,
    'ioncore-node': version
  }
};
...
/**
 * Will create a base ioncore Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/ioncore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a ioncore Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the ioncore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the ioncore-node directory and configuration
