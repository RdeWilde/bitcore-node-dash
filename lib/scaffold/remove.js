...
/**
 * Will remove a service from ioncore-node.json
...
/**
 * Will remove the Node.js service and from the ioncore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The ioncore-node configuration directory
...
function remove(options, done) {
  ...
  var ioncoreConfigPath = path.resolve(configPath, 'ioncore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(ioncoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a ioncore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to ioncore-node.json
        removeConfig(ioncoreConfigPath, service, next);
      });
    }, done
  );
}
