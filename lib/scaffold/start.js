...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and ioncore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/ioncore-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed ioncore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ioncore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in ioncore-node services directory
    ...
    if (servicePackage.ioncoreNode) {
      serviceModule = service.name + '/' + servicePackage.ioncoreNode;
    }
 ...
 * @param {Node} node - The ioncore Node instance
 */
